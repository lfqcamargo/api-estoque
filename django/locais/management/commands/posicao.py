...
from locais.models import Posicao
from datetime import datetime
import random

class Command(BaseCommand):
    help = "Popula o Banco de Dados na Tabela POES(Posição)"
    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(1, 11):
            descricao = f"Posição {i}"
            posicao, created = Posicao.objects.get_or_create(
                poes_dsc=descricao,
                defaults={
                    'user_id': user,
                    'poes_dat_cria': datetime.now(),
                    'poes_dat_atua': datetime.now()
                }
            )
